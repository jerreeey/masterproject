...
function createBlockNotice(blockClientId, result) {
  ...
  if (result.compliance === "AA") {
    if (result.size === "large") {
      message = `The text in the block ${blockClientId} does not comply with WCAG AA. The current contrast ratio is ${result.contrast.toFixed(2)}. For the current text size (more than or equal to 18pt or 14pt bold), the contrast ratio should be at least 3:1.`;
    } else {
      message = `The text in the block ${blockClientId} does not comply with WCAG AA. The contrast ratio is ${result.contrast.toFixed(2)}. For the current text size (less than 18pt or 14pt bold), the contrast ratio should be at least 4.5:1.`;
    }
  } else if (result.compliance === "AAA") {
    if (result.size === "large") {
      message = `The text in the block ${blockClientId} does not comply with WCAG AAA. The current contrast ratio is ${result.contrast.toFixed(2)}. For the current text size (more than or equal to 18pt or 14pt bold), the contrast ratio should be at least 4.5:1.`;
    } else {
      message = `The text in the block ${blockClientId} does not comply with WCAG AAA. The contrast ratio is ${result.contrast.toFixed(2)}. For the current text size (less than 18pt or 14pt bold), the contrast ratio should be at least 7:1.`;
    }
  }
    ...
    blocks.map(async (block) => {
      if (block.name === "core/cover" && block.attributes.id) {
        ...
        try {
          ...
          const overlayData = createOverlayCanvas(
            overlayText,
            img.width,
            img.height,
            position
          ...
          // Get the text color and convert it to an array of color channels
          const textColor = window.getComputedStyle(overlayText, null).getPropertyValue("color").replace(/[^\d,]/g, '').split(',')

          const result = calculateContrast(
            imgData,
            overlayData,
            isLargeText(overlayText),
            textColor
          );
